...
public class SaveService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent.getBooleanExtra("USER_CANCELLED", false) || intent.getBooleanExtra(
                "USER_CANCELLED_ALL",
                false)) {
            ...
            try {
                ...
                if (pageSaver.isCancelled() || !success) {
                    ...
                    if (pageSaver.isCancelled()) { //user cancelled, remove the notification, and delete files.
                        Log.e(
                                "SaveService",
                                "Stopping Service, (Cancelled). Deleting files in: " + destinationDirectory + ", from: " + pageUrl);
                        notificationTools.cancelAll();
                        stopService();
                    } else if (!success) { //something went wrong, leave the notification, and delete files.
                        Log.e(
                                "SaveService",
                                "Failed. Deleting files in: " + destinationDirectory + ", from: " + pageUrl);
                    }
                    return;
                }
                ...
                new Database(SaveService.this).addToDatabase(
                ...
                if (sharedPreferences.getBoolean("generate_saved_page_thumbnails", true)) {
                    ...
                    i.putExtra(
                            ...
                            "file://" + newSavedPageDirectory.getPath() + File.separator + "index.html");
                    i.putExtra(Database.ORIGINAL_URL, pageUrl);
                    i.putExtra(
                            Database.THUMBNAIL,
                            newSavedPageDirectory + File.separator + "saveForOffline_thumbnail.png");
                    startService(i);
                }

                stopService();

                notificationTools.notifyFinished(
                        pageSaver.getPageTitle(),
    ...
    private class PageSaveEventCallback implements EventCallback {
        ...
        public void onProgressChanged(
            ...
            notificationTools.updateProgress(
                    progress,
                    maxProgress,
                    indeterminate,
