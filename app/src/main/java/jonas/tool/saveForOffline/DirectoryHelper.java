...
public class DirectoryHelper {
    ...
    public static String getDestinationDirectory(SharedPreferences sharedPref) {
        ...
        if (sharedPref.getBoolean("is_custom_storage_dir", false)) {
            String directoryLocation = sharedPref.getString(
                    "custom_storage_dir",
                    defaultFileLocation) + createUniqueFilename() + File.separator;
    ...
    private static void createNomediaFile(String directoryLocation) {
        ...
        if (!directory.exists()) {
            if (!directory.mkdirs()) {
                Log.e(
                        "SaveService / DirectoryHelper",
                        "Could not create directory " + directoryLocation + ", mkdirs() returned false !");
            }
            try {
                ...
                Log.e(
                        "SaveService / DirectoryHelper",
                        "IOException while creating .nomedia file in " + directory.getParent() + " Is the path writable ?");
            }
        }
    }
