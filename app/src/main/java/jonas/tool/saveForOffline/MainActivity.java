...
public class MainActivity extends Activity implements SearchView.OnQueryTextListener {
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        if (newText.length() == 0) {
            ...
            if (gridAdapter.getCount() == 1) {
                actionbar.setSubtitle(R.string.one_search_result);
            } else if (gridAdapter.getCount() == 0) {
                actionbar.setSubtitle(R.string.no_search_results);
            } else {
                actionbar.setSubtitle(gridAdapter.getCount() + " " + getResources().getString(R.string.num_search_results));
            }
        }
        return false;
    }
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_sort_by:
                ...
                builder.setSingleChoiceItems(
                        R.array.sort_by,
                        DisplayAdapter.SortOrder.toInt(sortOrder),
    ...
    private void setUpGridClickListener() {
        ...
        mainGrid.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(
                    AdapterView<?> arg0, View clickedView, int position,
                    long id) {
                ...
                pageLoadDialog.show();
                String filepath = gridAdapter.getPropertiesByPosition(
                        position,
                        Database.FILE_LOCATION);
                ...
                if (defaultHtmlViewer) {
                    ...
                    try {
                        Intent i = new Intent(
                                ...
                                ViewActivity.class);
                        i.putExtra(
                                Database.ORIGINAL_URL,
                                gridAdapter.getPropertiesByPosition(
                                        position,
                                        Database.ORIGINAL_URL));
                        i.putExtra(
                                Database.TITLE,
                                gridAdapter.getPropertiesByPosition(position, Database.TITLE));
                        i.putExtra(
                                Database.ID,
                                gridAdapter.getPropertiesByPosition(position, Database.ID));
    ...
    class ModeCallback implements ListView.MultiChoiceModeListener {
            ...
            switch (item.getItemId()) {
                case R.id.action_rename:
                    ...
                    if (gridAdapter.selectedViewsPositions.size() == 1) {
                        e.setText(gridAdapter.getPropertiesByPosition(gridAdapter.selectedViewsPositions.get(
                                0), Database.TITLE));
                        e.selectAll();
                    ...
                    rename_dialog.setPositiveButton(
                            "Rename",
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    for (Integer position : gridAdapter.selectedViewsPositions) {
                                        ...
                                        dataBase.update(
                                                Database.TABLE_NAME,
                                                values,
                                                Database.ID + "=" + gridAdapter.getPropertiesByPosition(
                                                        position,
                                                        Database.ID),
                                                null);
                                    }
                                    ...
                                    if (gridAdapter.selectedViewsPositions.size() == 1) {
                                        ...
                                        Toast.makeText(
                                                MainActivity.this,
                                                "Renamed " + gridAdapter.selectedViewsPositions.size() + " saved pages",
                                                Toast.LENGTH_LONG).show();
                                    }
                    ...
                    rename_dialog.setNegativeButton(
                            "Cancel",
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    mode.finish();
                ...
                case R.id.action_delete:
                    ...
                    if (gridAdapter.selectedViewsPositions.size() == 1) {
                        build.setMessage("Do you want to delete ?\r\n" + gridAdapter.getPropertiesByPosition(
                                gridAdapter.selectedViewsPositions.get(0),
                                Database.TITLE));
                    } else {
        ...
        private class deleteItemsTask extends AsyncTask<Object, Integer, Integer> {
            ...
            protected Integer doInBackground(Object[] selectedPositions) {
                ...
                for (final Object position : selectedPositions) {
                    String fileLocation = gridAdapter.getPropertiesByPosition(
                            (Integer) position,
                            Database.FILE_LOCATION);
        ...
        public void onItemCheckedStateChanged(
            ...
            View gridCellLayout = mainGrid.getChildAt(position - mainGrid.getFirstVisiblePosition()).findViewById(
                    R.id.gridCellLayout);
            if (checked) {
